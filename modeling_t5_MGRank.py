...
# ================================================================
# Modifications for MGRank 
# ------------------------------------------------
# This file has been *manually modified* to enable custom attention mechanisms
# and Gaussian-based attention weighting for the MGRank framework.
#
# We have modified the `T5Attention` class in `modeling_t5.py` to implement:
# All original code is copyright Google Inc. and HuggingFace Inc. (Apache 2.0 License).
# ================================================================
"""PyTorch T5 model."""
...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_states = query_states.view(batch_size, -1, self.n_heads, self.key_value_proj_dim).transpose(1, 2)  # (batch_size, n_heads, seq_length, key_value_proj_dim).
        ...
        ###############################################################
        # Below: Custom attention weighting for keyphrase extraction (core part)
        def vanila_gaussian_weight(key_length, batch_size, t5_cand_pos_s, t5_cand_pos_e, layer_idx, initial_std, std_scaling_factor, device):
            ...
            mask_in_range = (x >= t5_cand_pos_s) & (x <= t5_cand_pos_e)  
        ...
        def custom_cross_attention_processing(
            ...
            Vanila Gaussian weighting, Mixture weighting, and top-k important document tokens.
            """
            ...
            combined_weight = gaussian_mixture_weighting(
                scores, 3, gaussian_weight, sim_word_idx_tensor
            )
        ...
        # ----- [END of custom candidate-aware weighting for cross-attention] ----- 
        #########################################################
...
class T5Block(nn.Module):                
    def __init__(self, config, has_relative_attention_bias=False, layer_idx: Optional[int] = None):
        ...
        self.layer.append(T5LayerFF(config))  # feed-forward
    ...
    def forward(
        ...
        **kwargs,
        ...
        do_cross_attention = self.is_decoder and encoder_hidden_states is not None # do cross_attention_outputs
...
class T5ForConditionalGeneration(T5PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: T5Config):
        super().__init__(config)
        print('weighted cross-attention t5 version')
